# 温馨提示：仅在北京时间 06:00 - 次日 01:00 之间运行检测逻辑 其他时间段不保活
Backup4App 容器自动重新部署。
...
- 仅在北京时间 `06:00 - 次日 01:00` 之间运行检测逻辑
- 第一次进入工作时段时，会先主动重新部署一次，用来建立后续循环的时间锚点
- 每次成功重新部署后，等待 55 分钟再进入 10 分钟检测窗口
- 检测窗口内每分钟检查一次 Back4App 应用状态
- 如果应用状态是 `EXPIRED`，立即触发重新部署并重置下一轮计时
...
- 北京时间 `01:00 - 06:00` 为静默时段，不执行检测和部署
- 每次成功重新部署后，会等待 55 分钟
- 55 分钟后只开启 10 分钟检测窗口
- 检测窗口之外不会访问 Back4App API
